String convertStringToSlug(str) {
...
filterList(List list, String text) {
  List filteredList = [];
  list.forEach((element) {
    int index = list.indexOf(element);
    if (convertStringToSlug(text).split('-').every((element) {
      return convertStringToSlug(list[index]).contains(element);
    })) filteredList.add(element);
  });
  return filteredList;
}

filterListMap(List<Map<dynamic, dynamic>> list, String text) {
  List<Map<dynamic, dynamic>> filteredList = [];
  list.forEach((element) {
    int index = list.indexOf(element);
    if (convertStringToSlug(text).split('-').every((element2) {
      return convertStringToSlug(list[index]['name']).contains(element2);
    })) {
      filteredList.add(element);
    }
    ;
...
String getNameFromList_byValue(List<Map<dynamic, dynamic>> list, int value) {
  var rs = list.where((item) => item['value'] == value).toList();
  return rs[0]['name'];
}
...
class Province {
  List renderProvinceArray() {
    ...
    initialProvinceList =
        PROVINCES.entries.map((entry) => entry.value['name']).toList();
  ...
  String getProvinceByCode(int code) {
    if (code.toString().length == 1) {
      return PROVINCES['0' + code.toString()] == null
          ? PROVINCES['01']['name']
          : PROVINCES['0' + code.toString()]['name'];
    }
    return PROVINCES[code.toString()] == null
        ? PROVINCES['01']['name']
        : PROVINCES[code.toString()]['name'];
  }

  int getCodeOfProvince(String name) {
    var fullProvinceList = [];
    PROVINCES.values.forEach((val) => fullProvinceList.add(val));
...
class District {
  List getDistrictListByProvinceCode(int code) {
    var fullDistrictList = [];
    DISTRICTS.values.forEach((val) => fullDistrictList.add(val));
    return fullDistrictList
        .where((district) => int.parse(district['parent_code']) == code)
        .map((districtMap) => districtMap['name'])
        .toList();
  }

  String getDistrictByCode(int code) {
    if (code.toString().length == 1) {
      return DISTRICTS['00$code']['name'] ?? '';
    } else if (code.toString().length == 2) {
      return DISTRICTS['0$code']['name'];
    }
    return DISTRICTS[code.toString()]['name'] ?? '';
  }

  int getCodeOfDistrict(String name) {
    var fullDistrictList = [];
    DISTRICTS.values.forEach((val) => fullDistrictList.add(val));
    return int.parse(fullDistrictList
        .where((district) => district['name'] == name)
        .toList()[0]['code']);
  }
}
